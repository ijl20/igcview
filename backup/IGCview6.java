...
class Config implements Serializable
{
  ...
  ConfigColour TPCOLOUR = new ConfigColour(0,1);               // vertical lines in ALT windows
...
class MapFrame extends Frame {
  ...
  private static String [] [] menus = {
    ...
    {"Task", "Define Task"},
    {"Replay", "Synchro Start", "Real Time"},
  ...
  int log_index_to_x(TrackLog log, int i)
    {
      ...
      while (log.tps_rounded>=next_tp && log.tp_index[next_tp]<i)
        {
          next_tp++;
          tp_dist += IGCview.task.dist[next_tp];
        }
