...
class TrackLog
{
    ...
    try { BufferedReader in = new BufferedReader(new FileReader(filename));
		...
		if (altitude[record_count]!=(float)0.0) baro_ok=true;
            ...
            else if (buf.startsWith("HFGID"))
	           {
                     ...
                     if (name_pos > 4 && name_pos+2 < buf.length())
                       name = buf.substring(name_pos+2);
                     System.out.println("Using name <"+name+"> from IGC file");
		   }
          }
          if (name==null) // if no HFGID record then get name from filename
            {
  ...
  public void draw(Graphics g, int num)
  {
    ...
    while (++i < max_i)
    {
        ...
        x2 = map_frame.long_to_x(longitude[i]);
        y2 = map_frame.lat_to_y(latitude[i]);
...
class Zoom
{
  ...
  void zoom_reset()
    {
      ...
      yscale = IGCview.config.start_xscale * 
  ...
  void zoom_to_task()   // zoom such that map displays task
	...
	for (int tp=2; tp<=IGCview.task.tp_count; tp++)
      ...
      if (dist_x>dist_y)
        {
          System.out.println("Task is wide.");
        ...
        {
          System.out.println("Task is tall.");
  ...
  void set_canvas()
    {
      switch (graph_type)
        {
          case IGCview.GRAPHALT:
                 if (IGCview.config.convert_altitude==(float)1.0)
                   ...
                   canvas = new AltCanvas (this, 6*width, 3*height, 
					   IGCview.MAXALT, 3048, 1524); // meters
                 break;
	  case IGCview.GRAPHCLIMB:
                 if (IGCview.config.convert_climb==(float)1.0)
                   ...
                   canvas = new ClimbCanvas (this, 6*width, 2*height,
					     IGCview.MAXCLIMB, 5, 1); // m/s
                 break;
          case IGCview.GRAPHCRUISE :
                 if (IGCview.config.convert_speed==(float)1.0)
                   canvas = new CruiseCanvas (this, 6*width, 2*height,
					      IGCview.MAXCRUISE, 50, 10); // knots
                 else if (IGCview.config.convert_speed==(float)1.0)
                   canvas = new CruiseCanvas (this, 6*width, 2*height,
					      IGCview.MAXCRUISE, 50, 10); // km/h
                 else
                   canvas = new CruiseCanvas (this, 6*width, 2*height,
					      IGCview.MAXCRUISE, 50, 10); // mph
                 break;
	  ...
	  while (cont)
	    {
	      ...
	      try {sleep(IGCview.config.RACEPAUSE);}
