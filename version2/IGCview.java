...
Each logpoint for time, altitude, latitude, longitude are in:
...
{
  ConfigVersion() {};
  String version = "2.03 - 26/Mar/1998";
}
...
class Config implements Serializable
{
  ...
  // WINDOWS
  int MAINWIDTH = 700;       // window default sizes
  int MAINHEIGHT = 500;
  int ALTWIDTH = 600;
  int ALTHEIGHT = 300;
  int SELECTWIDTH = 400;
...
public class IGCview extends Applet {
  ...
  static final int   MAXTPS = 8;           // maximum TPs in Task
  ...
  static final int   MAXTPDB = 5000;       // max number of TPs in database

  // graph window constants (i.e. altitude, climb, cruise windows)

  static final int   BASELINE = 5;         // offset of zero axis from bottom of graph frame
  ...
  static RulerFrame ruler_window = null;               // window for ruler data
  ...
  static TP_db tps = new TP_db();                // tp database
  ...
  static String places(float f, int d)
    {
       ...
       if (d == 0)
         ...
         p = String.valueOf( (float) ((int)(f*10+(float)0.5)) / (float) 10);
       else if ( d == 2)
         p = String.valueOf( (float) ((int)(f*100+(float)0.5)) / (float) 100);
       else if ( d == 3)
         p = String.valueOf( (float) ((int)(f*1000+(float)0.5)) / (float) 1000);
       return (p+"0000").substring(0,p.indexOf('.')+d+1);
    }
  ...
  static String make_title(TrackLog log)
    {
      if (log==null) return "";
      String title = IGCview.format_date(log.date)+" "+log.name+" (vs. ";
...
class MapFrame extends Frame implements Drawable, WindowListener
{
  Zoom zoom;
  ...
  private static String [] [] menus = {
    {"File", "Clear Logs", "Startup Configuration", "Save primary log as IGC file", "Exit"},
  ...
  MapFrame ()
    {
      load_config();
      set_title();
      width = IGCview.config.MAINWIDTH;
      height = IGCview.config.MAINHEIGHT;
  ...
  private void load_config()
    {
          try
	        ...
	        ObjectInputStream o = new ObjectInputStream (s);
              ...
              {
                System.out.println("Your "+IGCview.CONFIGFILE+" configuration file is the wrong version.");
                System.out.println("Save a new config file. Now using defaults.");
	      ...
	      // IGCview.map_frame.canvas.paint(IGCview.map_frame.canvas.getGraphics());
            ...
            {
              System.out.println("Unable to load "+IGCview.CONFIGFILE+". Using defaults.");
      	      IGCview.config = new Config();
            }
    }
                                              ...
                                              RacingCanvas
{
  ...
  int rect_x0 = 0, rect_y0 = 0;   // old draw x,y coordinates
  ...
  public void mousePressed (MouseEvent e)
    {
      if (ruler)
        {
          if (IGCview.ruler_window==null) IGCview.ruler_window = new RulerFrame();
          Graphics g = getGraphics();
          g.setColor(IGCview.config.MAPBACKGROUND.colour);
          g.setXORMode(Color.white);
          g.drawLine(rect_x, rect_y, rect_x0, rect_y0);  // remove old line
        }
      rect_x = e.getX();
      rect_y = e.getY();
      rect_x0 = rect_x;
  ...
  public void mouseReleased (MouseEvent e)
    { 
      ...
      float x_adj = ((float) map_frame.getSize().width / (float) (rect_x1-rect_x));
      float y_adj = ((float) map_frame.getSize().height / (float) (rect_y1-rect_y));
  ...
  void update_ruler()
    {
      ...
      IGCview.ruler_window.latitude = lat2;
      IGCview.ruler_window.longitude = long2;
      IGCview.ruler_window.dist = IGCview.dec_to_dist(lat1,long1,lat2,long2);
      IGCview.ruler_window.track = IGCview.dec_to_track(lat1,long1,lat2,long2);
      IGCview.ruler_window.update();
    }
  ...
  public void update (Graphics g)
    {
      ...
      if (ruler)
        {
          if (rect_x0!=0) g.drawLine(rect_x, rect_y, rect_x0, rect_y0);  // remove old line
          g.drawLine(rect_x, rect_y, rect_x1, rect_y1);    // draw new line
          rect_x0 = rect_x1;
          rect_y0 = rect_y1;  // save dimensions of drawn line
        }
      else if ((rect_x1-rect_x) > 0) 
        {
          g.drawRect(rect_x, rect_y, rect_x0-rect_x, rect_y0-rect_y);  // remove old rectangle
          g.drawRect(rect_x, rect_y, rect_x1-rect_x, rect_y1-rect_y);  // draw new rectangle
          rect_x0 = rect_x1;
          rect_y0 = rect_y1;  // save dimensions of drawn rectangle
        }
    }
  ...
  public void paint (Graphics g)
    { 
      ...
      if (IGCview.task!=null && IGCview.config.draw_sectors) IGCview.task.draw_sectors(g,this.map_frame);
      if (racing)
        {
          draw_legend(g);
          return;
      ...
      for (int i = 1; i <= IGCview.log_count; i++)
        {
          ...
          if (IGCview.secondary[i] && IGCview.logs[i]!=null)
            IGCview.logs[i].draw(map_frame,g,i);
        }
      if (IGCview.primary_index>0) 
        {
  ...
  public void actionPerformed (ActionEvent e)
    {
      ...
      else if (s.equals ("scoring")) view_scoring();
      else if (s.equals ("tp detail")) view_tp_detail();
      else if (s.equals ("startup configuration")) view_config();
      else if (s.equals ("save primary log as igc file")) save_primary();
  ...
  void cancel_ruler()
    { 
      if (IGCview.ruler_window!=null)
        {
         IGCview.ruler_window.dispose();
         IGCview.ruler_window = null;
        }
      ruler = false;
    }
  ...
  private void save_primary()
   {
      if (IGCview.primary_index==0)
        {
          MsgBox m = new MsgBox("No logs loaded.");
          return;
        }
     FileDialog fd = new FileDialog (map_frame, "Save Ttrack log as IGC file", FileDialog.SAVE);
     fd.setDirectory(IGCview.config.LOGDIR);
     fd.show ();
     String filename = fd.getFile ();
     IGCview.config.LOGDIR = fd.getDirectory();
     if (filename != null) {
       try { TrackLog log = IGCview.logs[IGCview.primary_index];
             ...
             MsgBox m = new MsgBox("Log "+IGCview.format_date(log.date)+" "+log.name+
  ...
  private void select_tracks()
    {
      ...
      if (IGCview.select_window!=null)
        {
          MsgBox m = new MsgBox("Select tracks window already open.");
          return;
        }
      IGCview.select_window = new SelectFrame(map_frame);
     ...
     try { 
           IGCview.tps.load(IGCview.config.TPFILE);
            ...
            { System.err.println ("TP database load error "+e);
              MsgBox m = new MsgBox("Problem loading "+IGCview.config.TPFILE+".");
            }
   }
  ...
  private void view_flight_data()
    {
      ...
      if (IGCview.data_window!=null)
        {
          MsgBox m = new MsgBox("Flight Data window already open.");
          return;
        }
  ...
  private void view_scoring()
    {
      ...
      if (IGCview.scoring_window!=null)
        {
          MsgBox m = new MsgBox("Scoring Data window already open.");
          return;
        }
      if (IGCview.task==null)
        {
          MsgBox m = new MsgBox("A task must be defined for Scoring Data.");
          return;
        }
  ...
  private void view_config()
    {
      if (IGCview.config_window!=null)
        {
          MsgBox m = new MsgBox("Configuration window already open.");
          return;
        }
  ...
  void maggot_synchro()
    {
      if (racing)
        {
          MsgBox m = new MsgBox("Replay already in progress - select 'Cancel' to clear.");
          return;
        }
      if (IGCview.primary_index==0)
        {
  ...
  void maggot_cancel()
	...
	if (racing)
        ...
        {
          MsgBox m = new MsgBox("No replay in progress to cancel.");
        }    
    }
  ...
  public void draw_plane(Graphics g, int log_num, int i) // draw plane at log index i
    { 
      ...
      int x = map_frame.long_to_x(glider_long);
      int y = map_frame.lat_to_y(glider_lat);
	  ...
	  if (i==IGCview.primary_index || IGCview.secondary[i])
	    {
	    	log = IGCview.logs[i];
		if (IGCview.config.glider_icons)
              mark_icon(g,i,5,legend_y,0,(float)0);
            else
              mark_plane(g,i,5,legend_y,0);
		g.setColor(Color.black);
		g.setPaintMode();
		g.drawString(log.name, 12, legend_y+4);
		legend_y += 12;
	    }
	}
    }
...
class TrackLog
{
  ...
  int landed_index = 1;                                // log point on landout
  ...
  {
    // note: this procedure reads records from the file 'filename', and adds
            ...
            if (!is_gardown && buf.charAt(0) == 'B')
              { 
                ...
                if (record_count==IGCview.MAXLOG)
                  {
                    MsgBox m = new MsgBox("Too many records in "+filename+" (limit="+IGCview.MAXLOG+")");
                    break;
                  }
            ...
            if (!is_gardown && buf.charAt(0) == 'C' && c_record_count<=IGCview.MAXTPS+3)
              { 
                ...
                if (c_record_count>IGCview.MAXTPS+3)
                  {
                    MsgBox m = new MsgBox("Didn't get task from log. (>"+
                                          IGCview.MAXTPS+
            ...
            else if (!is_gardown && buf.startsWith("HFDTE"))
	         {
              ...
              { 
                ...
                latitude[record_count] = decimal_latlong(buf.substring(4,6),
                                                         buf.substring(7,9)+
                                                         buf.substring(10,13)+"."+
                                                         buf.substring(13,14),
                                                         buf.charAt(3));
                longitude[record_count] = decimal_latlong(buf.substring(16,19),
                                                          buf.substring(20,22)+
                                                          buf.substring(23,26)+"."+
                                                          buf.substring(26,27),
                                                          buf.charAt(15));
  ...
  String lat_to_igc(float latitude) // convert latitude into DDMMmmm (= DD degrees, MM.mmm minutes)
    {
  ...
  String long_to_igc(float longitude) // convert longitude into DDDMMmmm (= DDD degrees, MM.mmm minutes)
    {
  ...
  void set_primary()
    {
       IGCview.map_frame.set_title();
       if (task.tp_count>1 && !task.equals(IGCview.task))
         {
           MsgResponse r = new MsgResponse();
           MsgBox m = new MsgBox("Primary log contains new task - do you want to use it?", r);
           if (r.ok_hit) IGCview.new_task(task);
         }
    }
  ...
  private void draw_dist_alt(AltCanvas canvas, Graphics g)
    {
      ...
      for (int i = 1; i <= record_count; i++)
        {
 	    if (latitude[i]==(float)0.0) continue;
          if (next_tp<=tps_rounded && i==tp_index[next_tp] && next_tp<IGCview.task.tp_count) 
            {
              next_tp++;
  ...
  void draw_alt_tp_lines(GraphCanvas canvas, Graphics g)
    {
  ...
  void find_tp_times()
    {
      ...
      tps_rounded = 0;
      for (i=1; i<=record_count; i++) // scan forwards for all TPs except start
      ...
      if (current_tp>2)      // to find start scan backwards from
        imax = tp_index[2];          // tp 2
      else
        {                            // or from furthest log point from start
  ...
  int dist_to_index(float d)
    {
      ...
      tp_lat = IGCview.task.tp[next_tp].latitude;
      tp_long = IGCview.task.tp[next_tp].longitude;
      i_min = (next_tp==1) ? 1 : tp_index[next_tp-1];
      i_max = (tps_rounded>=next_tp) ? tp_index[next_tp] : record_count;
  ...
  float index_to_dist(int index)
    {
      ...
      while (next_tp<=tps_rounded &&
             index>=tp_index[next_tp] &&
      ...
      tp_lat = IGCview.task.tp[next_tp].latitude;
      tp_long = IGCview.task.tp[next_tp].longitude;
      dist = IGCview.dec_to_dist(latitude[index], longitude[index], tp_lat, tp_long);
      if (dist>IGCview.task.dist[next_tp]) dist = IGCview.task.dist[next_tp];
      return tp_dist-dist;
    }
  ...
  void calc_leg_climb_data()  // calculate leg_average_climb[i], leg_climb_percent[i],
	...
	{
	  if (tps_rounded>0 && i<=thermal_count && thermal[i].finish_index<tp_index[1])
          { 
            i++;
            continue;
          } // pre-start
  ...
  void calc_leg_cruise_data()
    {
      float cruise_fraction;
      for (int leg=1;leg<=tps_rounded;leg++)
        {
          if (leg==IGCview.task.tp_count) break;
          cruise_fraction = ((float) 100 - leg_climb_percent[leg]) / 100; 
          if (cruise_fraction==(float)0.0)
            leg_cruise_avg[leg] = (float)0.0;
          else
            leg_cruise_avg[leg] = leg_speed[leg] / cruise_fraction;
        }
      cruise_fraction = ((float) 100 - task_climb_percent) / 100; 
      if (cruise_fraction==(float)0.0)
        task_cruise_avg = (float)0.0;
      else
        task_cruise_avg = task_speed / cruise_fraction;
    }
  ...
  void add(TP new_tp)
    {
      if (tp_count==IGCview.MAXTPDB)
        {
          MsgBox m = new MsgBox("Adding "+new_tp.trigraph+
...
class TP
{
  ...
  void igc_tp(String igc_rec)
    {
      trigraph = igc_rec.substring(18).toUpperCase();
      full_name = igc_rec.substring(18);
      latitude = decimal_latlong(igc_rec.substring(1,3),
                                 igc_rec.substring(3,5)+"."+
                                 igc_rec.substring(5,8),
                                 igc_rec.charAt(8));
      longitude = decimal_latlong(igc_rec.substring(9,12),
                                          ...
                                          WindowListener
{
  ...
  private float [] dist = new float [IGCview.MAXTPS+1]; // leg length in nm
      ...
      if ((TextField) e.getSource()==search) // <return> hit in search field
        {
          ...
          for (int i=1;i <= IGCview.tps.tp_count; i++)
            {
              if ((IGCview.tps.tp[i].trigraph+IGCview.tps.tp[i].full_name).indexOf(search_string)!= -1)
  ...
  void ok_hit()
    { int i=1;
      ...
      while (i<=IGCview.MAXTPS && tp_index[i] > 0)
        {
          task.add(IGCview.tps.tp[tp_index[i]]);
          i++;
        }
      IGCview.new_task(task);
    }

  void calc_length()  // sets length (nm) and tracks of task
    {
      ...
      for (int i=2; i<=IGCview.MAXTPS; i++)   // calculate leg lengths and tracks
        {
          ...
          dist[i] = IGCview.dec_to_dist(lat1, long1, lat2, long2);
          track[i] = IGCview.dec_to_track(lat1, long1, lat2, long2);
          tp_dist[i].setText(IGCview.places(dist[i]*IGCview.config.convert_task_dist,2));
          tp_track[i].setText(IGCview.places(track[i],1));
          length = length + dist[i];
          tp_length.setText(IGCview.places(length*IGCview.config.convert_task_dist,2));
          lat1 = lat2;
          long1 = long2;
        }
      for (int i=2; i<IGCview.MAXTPS; i++)   // calculate bisectors
        {
          ...
          bisector[i] = Task.calc_bisector(track[i], track[i+1]);
          tp_bisector[i].setText(IGCview.places(bisector[i],1));
        }
      return;
    }
...
class Task
{
  ...
  void add(TP new_tp)
    {
      ...
      track[tp_count] = IGCview.dec_to_track(tp[tp_count-1].latitude,
                                             tp[tp_count-1].longitude,
                                             new_tp.latitude,
                                             new_tp.longitude);
      dist[tp_count] = IGCview.dec_to_dist(tp[tp_count-1].latitude,
                                             tp[tp_count-1].longitude,
                                             new_tp.latitude,
                                             new_tp.longitude);
      if (tp_count>1)
        bisector[tp_count-1] = calc_bisector(track[tp_count-1], track[tp_count]);
      length = (float)0.0;
  ...
  public void draw(Graphics g)
    {
      ...
      x1 = IGCview.map_frame.long_to_x(tp[1].longitude);
      y1 = IGCview.map_frame.lat_to_y(tp[1].latitude);
      for (int i = 2; i <= tp_count; i++)
        {
          x2 = IGCview.map_frame.long_to_x(tp[i].longitude);
          y2 = IGCview.map_frame.lat_to_y(tp[i].latitude);
          g.drawLine(x1,y1,x2,y2);
          x1 = x2;
          y1 = y2;
        }
  ...
  void draw_sector(Graphics g, Drawable d, int tp_number)
    {
      ...
      if (tp_number==1)
        ...
        {
          tp_radius = IGCview.dist_to_pixels(d,latitude,longitude,IGCview.config.tp_radius);
  ...
  boolean in_sector(float latitude, float longitude, int tp_index)
    {
      ...
      if (tp_index == 1)
        ...
        {
          if (dist>IGCview.config.tp_radius) return false;  // not in sector if outside radius
  ...
  boolean equals(Task t)
	...
	{
          if (t.tp[i].latitude != tp[i].latitude) return false;
          if (t.tp[i].longitude != tp[i].longitude) return false;
        }
      return true;
    }
}
...
class Zoom
{
  ...
  void scale(float o_lat, float o_long, float factor)   // adjust scale by 'factor'
    {
      float adj_factor=factor;
      if (xscale*factor>20000) adj_factor = (float) 20000 / xscale;
      xscale *= adj_factor;
      yscale *= adj_factor;
      latitude = o_lat;
      longitude = o_long;
      if (adj_factor > (float) 1)
        { // zooming IN
...
class GraphFrame extends Frame implements WindowListener
{
  ...
  GraphFrame (int graph_type, TrackLog log)
    {
      this.graph_type = graph_type;
      this.log = log;
      width = IGCview.config.ALTWIDTH;
      height = IGCview.config.ALTHEIGHT;
      ...
      this.pack ();
      pane.doLayout();
      pane.setScrollPosition(0,canvas.height-pane.getViewportSize().height);
      repaint();
    }
                                            ...
                                            KeyListener
{
  ...
  public void actionPerformed (ActionEvent e)
    {
      ...
      else if (s.equals ("expand")) expand();
      else if (s.equals ("compress")) compress();
      else if (s.equals ("synchro start")) maggot_synchro();
      else if (s.equals ("real time")) maggot_real_time();
      else if (s.equals ("cancel")) maggot_cancel();
    }
  ...
  void set_time_start()
    {
      int i=1;
      if (frame.log.tps_rounded>0)   // graph from 1 hour before start, round down to nearest hour
        time_start = (frame.log.time[frame.log.tp_index[1]] / 3600 - 1) * 3600; 
      else
        {                           // skip to 3 mins after start of log
          while (i<frame.log.record_count && frame.log.time[i]-frame.log.time[1]<180) i++;
          time_start = (frame.log.time[i] / 3600) * 3600;
        }
    }
  ...
  void set_dist_x_axis()
        {
          if (IGCview.task==null || !frame.log.gps_ok)
            {
              MsgBox m = new MsgBox("Cannot view by distance if no task defined or no GPS data in log");
              return;
            }
  ...
  void draw_time_contours(Graphics g)
    {
      int line_x, line_y;
      for (int i=time_start; i < time_start+IGCview.GRAPHTIME; i += 3600)
        {
          line_x = time_to_x(i);
  ...
  void draw_dist_contours(Graphics g)
    {
      int line_x, line_y;
      for (int i=0; i<(IGCview.ALTMAXDIST*IGCview.config.convert_task_dist); i += 100)
        {
  ...
  int index_to_x(int i)
    {
      return log_index_to_x(IGCview.logs[IGCview.primary_index], i);
    }
  ...
  public void mousePressed (MouseEvent e)
    {
      ...
      if (x_axis==IGCview.TIMEX)
        {
          frame.log.move_cursor1_time(x_to_time(x));
        }
      else
        {
	  frame.log.move_cursor1_dist(x_to_dist(x));
        }
      if (IGCview.cursor1<frame.log.record_count)
        i = IGCview.cursor1+1;
      else
        i = frame.log.record_count;
      IGCview.map_frame.canvas.move_cursor2(i);
      draw_cursors(g);
    }
  ...
  public void mouseDragged (MouseEvent e)
    {
      ...
      if (x_axis==IGCview.TIMEX)
        {
          frame.log.move_cursor2_time(x_to_time(cursor2_x));
        }
      else
        {
	  frame.log.move_cursor2_dist(x_to_dist(cursor2_x));
        }
    }
  ...
  void draw_cursors(Graphics g)
    {
      cursor1_x = index_to_x(IGCview.cursor1);
      cursor2_x = index_to_x(IGCview.cursor2);
  ...
  void graph_cursor_right()
    {
      Graphics g = getGraphics();
      if (cursor2_x!=cursor1_x) draw_cursor(g, cursor2_x); // erase previous cursor2
      IGCview.map_frame.canvas.move_cursor2(IGCview.cursor2+1);
      cursor2_x = index_to_x(IGCview.cursor2);
      if (cursor1_x!=cursor2_x) draw_cursor(g, cursor2_x);
    }
  ...
  void graph_insert()
    {
      Graphics g = getGraphics();
      IGCview.map_frame.canvas.move_cursor1(IGCview.cursor2);
      IGCview.map_frame.canvas.map_cursor_right();
      draw_cursor(g,cursor1_x); // erase previous cursors
      if (cursor2_x!=cursor1_x) draw_cursor(g, cursor2_x); 
      draw_cursors(g);
    }
  ...
  void graph_home()
    {
      Graphics g = getGraphics();
      IGCview.map_frame.canvas.move_cursor1(1);
  ...
  void graph_end()
    {
      Graphics g = getGraphics();
      TrackLog log = IGCview.logs[IGCview.primary_index];
      IGCview.map_frame.canvas.move_cursor2(log.record_count-1);
      draw_cursor(g,cursor1_x);                             // erase previous cursor1
      if (cursor2_x!=cursor1_x) draw_cursor(g, cursor2_x);  // erase previous cursor2
      draw_cursors(g);      
    }
  ...
  public void paint (Graphics g)
    { 
      ...
      if (!racing)
        ...
        frame.log.draw_alt_tp_lines(this,g);
    }
}
...
class CursorFrame extends Frame implements WindowListener
{
  ...
  CursorFrame ()
    {
      ...
      width = IGCview.config.CURSORWIDTH;
      height = IGCview.config.CURSORHEIGHT;
      setSize(width,height);
      this.addWindowListener(this);
      ScrollPane pane = new ScrollPane(ScrollPane.SCROLLBARS_AS_NEEDED);
      this.add (pane, "Center");
...
class CursorWindow extends Panel implements ActionListener
{
  ...
  private Label task_dist_heading;
  private float task_dist1=(float)0.0;
  private Label task_dist1_label = new Label("000.00");
  private float task_dist2=(float)0.0;
  private Label task_dist2_label = new Label("000.00");
  ...
  CursorWindow(CursorFrame frame)
    {
      ...
      task_dist_heading = new Label("Total Dist "+s);
      task_dist_heading.setAlignment(Label.LEFT);
      l_add(task_dist_heading);
      task_dist1_label.setAlignment(Label.LEFT);
      l_add(task_dist1_label);
      c.gridwidth = GridBagConstraints.REMAINDER;
      task_dist2_label.setAlignment(Label.LEFT);
      l_add(task_dist2_label);
  ...
  public void update()
    {
      calc_values();
      task_dist1_label.setText(IGCview.places(task_dist1*IGCview.config.convert_task_dist,2));
      task_dist2_label.setText(IGCview.places(task_dist2*IGCview.config.convert_task_dist,2));
...
class RulerFrame extends Frame implements WindowListener
{
  ...
  float latitude=(float)0.0, longitude=(float)0.0, dist=(float)0.0, track=(float)0.0;

  RulerFrame ()
    {
      this.setTitle("Ruler Data");
      width = IGCview.config.RULERWIDTH;
      height = IGCview.config.RULERHEIGHT;
      setSize(width,height);
      this.addWindowListener(this);
      ScrollPane pane = new ScrollPane(ScrollPane.SCROLLBARS_AS_NEEDED);
      this.add (pane, "Center");
      window = new RulerWindow(this);
      pane.add(window);
      pack ();
      show();
    }
  ...
  public void windowClosing(WindowEvent e)
    {
      IGCview.map_frame.canvas.ruler = false;
      if (IGCview.tp_window!=null) IGCview.tp_window.ruler = false;
      dispose();
	IGCview.ruler_window = null;
    }
...
class RulerWindow extends Panel
{
  private RulerFrame frame;              // frame to draw on

  private GridBagLayout gridbag;
  private GridBagConstraints c;

  private Label lat_label = new Label("--------------",Label.LEFT);
  private Label long_label = new Label("--------------",Label.LEFT);
  private Label dist_label = new Label("--------------",Label.LEFT);
  private Label track_label = new Label("--------------",Label.LEFT);
  String dist_units;

  RulerWindow(RulerFrame frame)
    {
      this.frame = frame;
      gridbag = new GridBagLayout();
      c = new GridBagConstraints();
      ...
      c.fill = GridBagConstraints.HORIZONTAL;
      c.gridwidth = GridBagConstraints.REMAINDER;
      lat_label.setAlignment(Label.LEFT);
      gridbag.setConstraints(lat_label, c);
      add(lat_label);
      long_label.setAlignment(Label.LEFT);
      gridbag.setConstraints(long_label, c);
      add(long_label);
      dist_label.setAlignment(Label.LEFT);
      gridbag.setConstraints(dist_label, c);
      add(dist_label);
      track_label.setAlignment(Label.LEFT);
      gridbag.setConstraints(track_label, c);
      add(track_label);
    }

  public void update()
    {
      lat_label.setText(IGCview.format_lat(frame.latitude));
      long_label.setText(IGCview.format_long(frame.longitude));
      dist_label.setText(IGCview.places(frame.dist*IGCview.config.convert_task_dist,3)+dist_units);
      track_label.setText(IGCview.places(frame.track,1)+" deg");
    }
}
...
class DataFrame extends Frame implements ActionListener, WindowListener
{
  ...
  void refresh()
    {
      if (IGCview.task==null)
        {
          MsgBox m = new MsgBox("A task must be defined for Flight Data.");
          dispose();
	    IGCview.data_window = null;
          return;
        }
        ...
        {
          MsgBox m = new MsgBox("Primary log has no GPS data (baro only?).");
          dispose();
	    IGCview.data_window = null;
          return;
        }
  ...
  void print_task_data()
	...
	(IGCview.config.convert_task_dist==(float)1.0 ? " nm" : " miles");
      ...
      if (log.tps_rounded==IGCview.task.tp_count)
        write(log.name+" Task Completed OK.  ");
      else
        write(log.name+" LANDED OUT.  ");
      write("Achieved distance: "+
            IGCview.places(log.task_distance*IGCview.config.convert_task_dist,2)+units+"\t("+
            IGCview.places(s.task_distance*IGCview.config.convert_task_dist,2)+units+")\n\n");
      write("Start\t\t\tFinish\t\t\tTask\n");
      write("Time:\t\t\tTime:\t\t\tTime:\n");
      write(IGCview.format_clock(log.tp_time[1])+"\t"+
            "("+IGCview.format_clock(s.tp_time[1])+")\t");
      if (log.tps_rounded==IGCview.task.tp_count)
	  write(IGCview.format_clock(log.tp_time[IGCview.task.tp_count])+"\t");
      else
        write("00:00:00\t");
      write("("+IGCview.format_clock(s.tp_time[IGCview.task.tp_count])+")\t"+
	...
	(IGCview.config.convert_speed==(float)1.0 ? "(knots)" : "(mph)");
      ...
      write(IGCview.places(log.task_speed*IGCview.config.convert_task_dist,2)+"\t"+
            "("+IGCview.places(s.task_speed*IGCview.config.convert_task_dist,2)+")\t"+
...
class ConfigWindow extends Panel implements ActionListener, FilenameFilter
{
  ...
  // WINDOWS
  private TextField MAINWIDTH = new TextField(String.valueOf(IGCview.map_frame.getSize().width));
  private TextField MAINHEIGHT = new TextField(String.valueOf(IGCview.map_frame.getSize().height));
  private TextField ALTWIDTH = new TextField(String.valueOf(
				   (IGCview.alt_window==null) ? IGCview.config.ALTWIDTH :
                                                                IGCview.alt_window.getSize().width));
  private TextField ALTHEIGHT = new TextField(String.valueOf(
				   (IGCview.alt_window==null) ? IGCview.config.ALTHEIGHT :
                                                                IGCview.alt_window.getSize().height));
  ...
  private TextField CURSORWIDTH = new TextField(String.valueOf(
				   (IGCview.cursor_window==null) ? IGCview.config.CURSORWIDTH :
                                                                IGCview.cursor_window.getSize().width));
  private TextField CURSORHEIGHT = new TextField(String.valueOf(
				   (IGCview.cursor_window==null) ? IGCview.config.CURSORHEIGHT :
                                                                IGCview.cursor_window.getSize().height));
  private TextField DATAWIDTH = new TextField(String.valueOf(
				   (IGCview.data_window==null) ? IGCview.config.DATAWIDTH :
                                                                IGCview.data_window.getSize().width));
  private TextField DATAHEIGHT = new TextField(String.valueOf(
				   (IGCview.data_window==null) ? IGCview.config.DATAHEIGHT :
                                                                IGCview.data_window.getSize().height));
  ...
  void draw()
    {
      ...
      set_text(MAINWIDTH, "Width of main map window (e.g. 700 pixels)");
      set_text(MAINHEIGHT, "Height of main map window (e.g. 500 pixels)");
      set_text(ALTWIDTH, "Width of graph windows (e.g. 600 pixels)");
      set_text(ALTHEIGHT, "Height of graph windows (e.g. 300 pixels)");
  ...
  void use_config()
    { 
      ...
      IGCview.config.MAINWIDTH = Integer.valueOf(MAINWIDTH.getText()).intValue();
      IGCview.config.MAINHEIGHT = Integer.valueOf(MAINHEIGHT.getText()).intValue();
      IGCview.config.ALTWIDTH = Integer.valueOf(ALTWIDTH.getText()).intValue();
      IGCview.config.ALTHEIGHT = Integer.valueOf(ALTHEIGHT.getText()).intValue();
      ...
      IGCview.map_frame.zoom = new Zoom(IGCview.config.start_latitude,
                                        IGCview.config.start_longitude,
                                        IGCview.config.start_xscale);
      IGCview.map_frame.canvas.paint(IGCview.map_frame.canvas.getGraphics());
      config_frame.dispose();
  ...
  private void save_config()
   {
    ...
    if (f != null) {
      try {
        IGCview.config.start_latitude = IGCview.map_frame.zoom.latitude;
        IGCview.config.start_longitude = IGCview.map_frame.zoom.longitude;
        IGCview.config.start_xscale = IGCview.map_frame.zoom.xscale;
      ...
      if (f != null)
        {
	      ...
	      FileInputStream s = new FileInputStream (fd.getDirectory()+f);
	      ObjectInputStream o = new ObjectInputStream (s);
            ConfigVersion c = (ConfigVersion) o.readObject();
              if (!c.version.substring(0,4).equals(IGCview.config_version.version.substring(0,4)))
              {
                MsgBox m = 
                  new MsgBox("Your "+IGCview.CONFIGFILE+" configuration file is the wrong version."+
                             " Using defaults.");
  	          IGCview.config = new Config();
                return;
              }
...
class WindWindow extends Dialog implements ActionListener, WindowListener
{
  int width, height;
...
class MsgResponse
{
  boolean ok_hit = false, cancel_hit = false;
}
...
class MsgBox extends Dialog implements ActionListener, WindowListener
{
  int width, height;
  ...
  MsgBox(String message, MsgResponse r)
    {
      super(IGCview.map_frame, "IGCview"); 
      this.addWindowListener(this);
      response = r;
  ...
  public void actionPerformed(ActionEvent e)
    {
      ...
      if (button_label.equals("OK"))
        { 
          if (do_cancel) response.ok_hit = true;
          dispose();
          return;
        }
      if (button_label.equals("Cancel"))
	{
          response.cancel_hit = true;
          dispose();
          return;
        }
    }
  ...
  public void windowClosing(WindowEvent e)
    {
      if (do_cancel) response.cancel_hit = true;
      dispose();
    }
  ...
  public void actionPerformed (ActionEvent e)
    {
      ...
      if (s.equals ("zoom out"))
        ...
        { 
          if (!IGCview.map_frame.canvas.ruler) cancel_ruler();
