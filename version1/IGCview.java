...
The number of records in a given tracklog is in the log variable 'record_count'.
...
{
  ConfigVersion() {};
  String version = "1.02 - 20/Feb/1998";
}
...
public class IGCview extends Applet {
  ...
  static final int   MAXALT = 40000;       // altitude window 0..MAXALT feet
  static final int   ALTMAXDIST = 750;     // altitude window 0..MAXALTDIST nm
  static final int   GRAPHTIME = 64800;    // graph width is 18 hours  
  ...
  static final int   LANDEDSPEED = 5;      // landed if speed below 5 knots
  ...
  static String places(float f, int d)
    {
       if (d == 0)
         ...
         return String.valueOf( (float) ((int)(f*1000+(float)0.5)) / (float) 1000);
       else return "????";
    }
  ...
  static private String three_digits(int i)
    {
...
class MapFrame extends Frame implements Drawable, WindowListener
{
  ...
  MapFrame (int width, int height)
    {
      this.setTitle("IGCview (Version "+IGCview.config_version.version+")");
      this.width = width;
      this.height = height;
      ...
      for (int i=2; i<=IGCview.MAXTPS; i++)   // calculate leg lengths and tracks
        {
          ...
          tp_dist[i].setText(IGCview.places(dist[i]*IGCview.config.convert_task_dist,1));
          tp_track[i].setText(IGCview.places(track[i],0));
          length = length + dist[i];
          tp_length.setText(IGCview.places(length*IGCview.config.convert_task_dist,1));
          lat1 = lat2;
          long1 = long2;
        }
